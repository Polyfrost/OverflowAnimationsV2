...
public class RenderItemMixin {
    ...
    private float modifyEffect(float original) {
        return OldAnimationsSettings.enchantmentGlint  && OverflowAnimations.oldAnimationsSettings.enabled ?
                OverflowAnimations.transform != ItemCameraTransforms.TransformType.GUI ? -5.0f : 0.0f : original;
    }

    @ModifyConstant(method = "renderEffect", constant = @Constant(floatValue = 10.0F))
    private float modifyEffect2(float original) {
        return OldAnimationsSettings.enchantmentGlint  && OverflowAnimations.oldAnimationsSettings.enabled ?
                OverflowAnimations.transform != ItemCameraTransforms.TransformType.GUI ? 10.0f : 0.0f : original;
    }
    ...
    private float modifyEffect3(float original) {
        return (OldAnimationsSettings.enchantmentGlint  && OverflowAnimations.oldAnimationsSettings.enabled &&
                OverflowAnimations.transform == ItemCameraTransforms.TransformType.GUI ? -1F : 1F) * original;
    }
    ...
    public void modifyEffect5(IBakedModel model, CallbackInfo ci) {
        if (OldAnimationsSettings.spritesGlint  && OverflowAnimations.oldAnimationsSettings.enabled &&
