...
public class ModelBipedMixin {
    ...
    //todo: consider field re-assignment
    ...
    private void overflowAnimations$reAssignArmPosition(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn, CallbackInfo ci) {
        if (!MainModSettings.INSTANCE.getOldSettings().getOldArmPosition() || !MainModSettings.INSTANCE.getOldSettings().enabled) { return; }
    ...
    @Redirect(
            ...
            at = @At(
                    value = "FIELD",
                    opcode = Opcodes.PUTFIELD,
                    target = "Lnet/minecraft/client/model/ModelRenderer;rotateAngleZ:F",
                    ordinal = 0
            )
    )
    private void overflowAnimations$removeField(ModelRenderer instance, float value) {
        if (MainModSettings.INSTANCE.getOldSettings().getWackyArms() && MainModSettings.INSTANCE.getOldSettings().enabled) { return; }
