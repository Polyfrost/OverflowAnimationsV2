...
public class ModelBipedMixin {
    ...
    private void removeField(ModelRenderer instance, float value) {
    ...
    private void removeField2(ModelRenderer instance, float value) {
        if (!OldAnimationsSettings.wackyArms || !OldAnimationsSettings.INSTANCE.enabled) {
    ...
    private void removeField3(ModelRenderer instance, float value) {
        if (!OldAnimationsSettings.wackyArms || !OldAnimationsSettings.INSTANCE.enabled) {
            bipedRightArm.rotateAngleZ = 0.0f;
        }
    }
    ...
    private void wackyArms(float limbSwing, float limbSwingAmount, float ageInTicks, float netHeadYaw, float headPitch, float scaleFactor, Entity entityIn, CallbackInfo ci) {
        if (OldAnimationsSettings.wackyArms && OldAnimationsSettings.INSTANCE.enabled) {
